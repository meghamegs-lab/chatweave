...
export function createPluginInstance(
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, '{}', 'active', ?, ?)
  `).run(id, sessionId, pluginId, now, now);
 ...
 */
export function updatePluginInstanceState(
  instanceId: string,
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(JSON.stringify(state), now, instanceId);
}
 ...
 */
export function processCompletion(completion: CompletionEvent): {
  ...
  db.prepare(`
    ...
    SET status = 'completed', completion_data = ?, updated_at = ?
    WHERE id = ?
  `).run(JSON.stringify({ event: completion.event, data: completion.data, summary: completion.summary }), now, completion.instanceId);
 ...
 */
export function getPluginInstances(sessionId: string): Array<{
  id: string;
  pluginId: string;
  status: string;
  state: Record<string, unknown>;
